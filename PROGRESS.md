...
| 2026-07-23 | Stage 4 ✅ | 9/9 통과. 전역 키스페이스(mutableMapOf), 명령 테이블(CommandSpec: arity+handler 람다), SET/GET, null bulk($-1), arity 검증. Commands.kt 분리. 첫 시도 버그 2개: GET 미스 이중 포장(?:를 폴백 대신 early return으로), -ERR 접두어 누락. 학습: 락 없는 공유 키스페이스(이벤트 루프 원자성 실증), 함수를 값으로 다루는 명령 테이블, lookupCommand 구조. (Stage 3의 SET name PING 회귀 케이스는 SET이 실명령이 되며 +OK가 정답 — 낡은 테스트) |
